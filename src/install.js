...
const optionalDependencies = ["@anthropic-ai/sdk", "@google/generative-ai"];
...
export const installOptionalDependencies = async () => {
    ...
    for (const dependency of optionalDependencies) {
        execSync(`npm install ${dependency} --no-save --prefix ${installPath}`, {
            stdio: "inherit",
        });
    }
...
export const getMissingOptionalDependencies = () => {
    return Promise.all(
        optionalDependencies.filter(async (dependency) => {
            try {
